...
def plot_res(model, data):
    ...
    ax.contourf(x, y, grid_res, cmap="plasma", alpha=0.4)

    inputs = data.test_loader.dataset[:][0]
    labels = data.test_loader.dataset[:][1]

    ax.scatter(inputs[:, 0], inputs[:, 1], c=labels, cmap="plasma", s=5)
...
def plot_server_res():
    ...
    response = requests.post(
        "http://127.0.0.1:5001/invocations",  #If you hosted locally on port 5001
        data=json_dump,
        headers={"Content-Type": "application/json"},
    )
    values = response.json()["predictions"]["sigmoid"] #Gets the output from the sigmoid layer
